...
@router.get('/property')
def get_filtered_properties(min_kamar_tidur: int, min_kamar_mandi: int, min_car_port: int, luas_tanah: int, luas_bangunan: int, minimal_harga: int, maksimal_harga: int, get_current_user: schemas.User = Depends(OAuth2.get_current_user)):
...
def get_by_id(id: int, get_current_user: schemas.User = Depends(OAuth2.get_current_user)):
    return get_id(id)


@router.get('/kpr/property/{id}')
def kpr_property_calculator(id: int, jangka_waktu: int, suku_bunga_fixed: float, masa_kredit_fix: int, suku_bunga_floating: float, get_current_user: schemas.User = Depends(OAuth2.get_current_user)):
...
def kpr_property_bank_calculator(id: int, bank: str, jangka_waktu: int, suku_bunga_floating: float, get_current_user: schemas.User = Depends(OAuth2.get_current_user)):
    return prop_bank_calc(id, bank, jangka_waktu, suku_bunga_floating)


# Need to fix for Enum
@router.get('/kpr/property/bank/{bank}')
def kpr_bank_calculator(harga: int, bank: str, jangka_waktu: int, suku_bunga_floating: float, get_current_user: schemas.User = Depends(OAuth2.get_current_user)):
    return bank_calc(harga, bank, jangka_waktu, suku_bunga_floating)


@router.get('/get-properties-kpr')
def get_properties_by_kpr(jangka_waktu: int, max_cicilan_awal: int, max_cicilan_akhir: int, get_current_user: schemas.User = Depends(OAuth2.get_current_user)):
